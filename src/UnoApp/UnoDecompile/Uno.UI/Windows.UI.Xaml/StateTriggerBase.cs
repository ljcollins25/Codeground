...
public class StateTriggerBase : DependencyObject, IDependencyObjectStoreProvider, IWeakReferenceProvider
{
	...
	public static DependencyProperty DataContextProperty { get; } = DependencyProperty.Register("DataContext", typeof(object), typeof(StateTriggerBase), new FrameworkPropertyMetadata(null, FrameworkPropertyMetadataOptions.Inherits, delegate(DependencyObject s, DependencyPropertyChangedEventArgs e)
	{
		((StateTriggerBase)s).OnDataContextChanged(e);
	}));
	...
	protected StateTriggerBase()
	{
		...
		this.RegisterParentChangedCallback(this, delegate
		{
			OnOwnerChanged();
		});
	}
	...
	internal void SetActivePrecedence(StateTriggerPrecedence precedence)
	{
		if (CurrentPrecedence == precedence)
		{
			...
			{
				this.Log().DebugFormat(string.Format("StateTrigger [{0}] (owner={1}/{2}) precedence:{3} [DUPLICATED: IGNORED]", GetType().Name, ((object)Owner?.Owner) ?? ((object)"<null>"), ((object)Owner) ?? ((object)"<null>"), precedence));
			}
			return;
		}
		if (this.Log().IsEnabled(LogLevel.Debug))
		{
			this.Log().DebugFormat(string.Format("StateTrigger [{0}] (owner={1}/{2}) precedence:{3}", GetType().Name, ((object)Owner?.Owner) ?? ((object)"<null>"), ((object)Owner) ?? ((object)"<null>"), precedence));
		}
