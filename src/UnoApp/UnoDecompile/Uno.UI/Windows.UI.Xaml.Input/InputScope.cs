...
public class InputScope : DependencyObject, IDependencyObjectStoreProvider, IWeakReferenceProvider
{
	...
	public static DependencyProperty DataContextProperty { get; } = DependencyProperty.Register("DataContext", typeof(object), typeof(InputScope), new FrameworkPropertyMetadata(null, FrameworkPropertyMetadataOptions.Inherits, delegate(DependencyObject s, DependencyPropertyChangedEventArgs e)
	{
		((InputScope)s).OnDataContextChanged(e);
	}));
	...
	{
		return Names.FirstOrDefault()?.NameValue ?? InputScopeNameValue.Default;
	}
