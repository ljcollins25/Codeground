...
public sealed class SplitViewTemplateSettings : DependencyObject, IDependencyObjectStoreProvider, IWeakReferenceProvider
{
	...
	public GridLength CompactPaneGridLength => new GridLength((float)CompactPaneLength, GridUnitType.Pixel);

	public GridLength OpenPaneGridLength => new GridLength((float)OpenPaneLength, GridUnitType.Pixel);

	public double NegativeOpenPaneLength => 0.0 - OpenPaneLength;

	public double NegativeOpenPaneLengthMinusCompactLength => NegativeOpenPaneLength - CompactPaneLength;

	public double OpenPaneLengthMinusCompactLength => OpenPaneLength - CompactPaneLength;

	public double OpenPaneLength { get; }

	public double CompactPaneLength { get; }

	public RectangleGeometry LeftClip => new RectangleGeometry
	{
		Rect = new Rect(0.0, 0.0, CompactPaneLength, ViewHeight)
	};

	public RectangleGeometry RightClip => new RectangleGeometry
	{
		Rect = new Rect(OpenPaneLengthMinusCompactLength, 0.0, CompactPaneLength, ViewHeight)
	};

	public double ViewHeight { get; internal set; } = 2000.0;
	...
	public static DependencyProperty DataContextProperty { get; } = DependencyProperty.Register("DataContext", typeof(object), typeof(SplitViewTemplateSettings), new FrameworkPropertyMetadata(null, FrameworkPropertyMetadataOptions.Inherits, delegate(DependencyObject s, DependencyPropertyChangedEventArgs e)
	{
		((SplitViewTemplateSettings)s).OnDataContextChanged(e);
	}));
