...
public class EasingFunctionBase : DependencyObject, IEasingFunction, IDependencyObjectStoreProvider, IWeakReferenceProvider
{
	...
	public static DependencyProperty EasingModeProperty { get; } = DependencyProperty.Register("EasingMode", typeof(EasingMode), typeof(EasingFunctionBase), new FrameworkPropertyMetadata(EasingMode.EaseOut));
	...
	public static DependencyProperty DataContextProperty { get; } = DependencyProperty.Register("DataContext", typeof(object), typeof(EasingFunctionBase), new FrameworkPropertyMetadata(null, FrameworkPropertyMetadataOptions.Inherits, delegate(DependencyObject s, DependencyPropertyChangedEventArgs e)
	{
		((EasingFunctionBase)s).OnDataContextChanged(e);
	}));
	...
	{
		throw new NotImplementedException("The member double EasingFunctionBase.Ease(double normalizedTime) is not implemented in Uno.");
	}
	...
	public virtual double Ease(double currentTime, double startValue, double finalValue, double duration)
	{
		if (currentTime != 0.0)
		{
			return Lerp(startValue, finalValue, currentTime / duration);
		}
		return startValue;
	}

	private static double Lerp(double first, double last, double by)
	{
		return first * (1.0 - by) + last * by;
	}
